...
///
/// `HKDFRng` uses the HMAC-based Key Derivation Function (HKDF) to generate deterministic
/// random numbers from a combination of key material and salt. It serves as a key-stretching
/// mechanism that can produce an arbitrary amount of random-looking bytes from a single seed.
///
/// Since it produces deterministic output based on the same inputs, it's useful for
/// situations where repeatable randomness is required, such as in testing or when
/// deterministically deriving keys from a master seed.
///
...
///
/// The implementation automatically handles buffer management, fetching new data
/// using HKDF as needed with an incrementing counter to ensure unique output
/// for each request.
...
impl HKDFRng {
    ...
    /// let mut rng = HKDFRng::new_with_page_length(b"my secure seed", "application-context", 64);
    ///
