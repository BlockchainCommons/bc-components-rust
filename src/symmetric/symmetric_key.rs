...
/// implementation follows the IETF ChaCha20-Poly1305 specification as defined in [RFC-8439](https://datatracker.ietf.org/doc/html/rfc8439).
///
/// Symmetric encryption uses the same key for both encryption and decryption,
/// unlike asymmetric encryption where different keys are used for each
/// operation.
///
/// `SymmetricKey` can be used to encrypt plaintext into an `EncryptedMessage`
/// that includes:
...
impl SymmetricKey {
    ...
    /// Create a new random symmetric key using the given random number
    /// generator.
    ...
    /// Encrypt the given plaintext with this key, and the given digest of the
    /// plaintext, and nonce.
...
impl AsRef<SymmetricKey> for SymmetricKey {
    fn as_ref(&self) -> &SymmetricKey { self }
}
...
impl CBORTagged for SymmetricKey {
    fn cbor_tags() -> Vec<Tag> { tags_for_values(&[tags::TAG_SYMMETRIC_KEY]) }
}

/// Implements conversion from SymmetricKey to CBOR for serialization.
impl From<SymmetricKey> for CBOR {
    fn from(value: SymmetricKey) -> Self { value.tagged_cbor() }
}
...
/// Implements `TryFrom<CBOR>` for SymmetricKey to support conversion from CBOR
/// data.
